...
python3 model_alignment.py -p ~/example/test.ply
...
def format_converter(model_path, output_directory):
    ...
    aligned_model_path = join(output_directory, base_name + '_aligned.ply')
    o3d.io.write_point_cloud(filename=aligned_model_path, pointcloud=pcd_r, write_ascii=True)

    # Save modelfilea as ascii format in xyz
    xyz_model_path = join(output_directory, base_name + '.xyz')
    o3d.io.write_point_cloud(filename=xyz_model_path, pointcloud=pcd_r, write_ascii=True)

    # check saved file
    if os.path.exists(aligned_model_path) and os.path.exists(xyz_model_path):
        print("Converted 3d models were saved at {0} and {1}\n".format(aligned_model_path, xyz_model_path))
    else:
...
if __name__ == '__main__':
    ...
    # make sure model file exists first
    ...
    format_converter(model_path, output_directory)
