...
python3 skeleton_analyze.py -m1 ~/example/test_skeleton.ply -m2 ~/example/test_aligned.ply -m3 ~/example/slices/ -v True
...
def analyze_skeleton(current_path, filename_skeleton, filename_pcloud):
    ...
    for idx, v_end in enumerate(end_vlist):
        ...
        end_v = [X_skeleton[int_v_list[0]] - X_skeleton[int_v_list[int(len(int_v_list) - 1.0)]],
                 Y_skeleton[int_v_list[0]] - Y_skeleton[int_v_list[int(len(int_v_list) - 1.0)]],
                 Z_skeleton[int_v_list[0]] - Z_skeleton[int_v_list[int(len(int_v_list) - 1.0)]]]
    ...
    if not (filename_pcloud is None):
        ...
        print("pt_diameter_max = {} pt_diameter_min = {} pt_diameter = {} pt_length = {} pt_volume = {}\n".format(pt_diameter_max, pt_diameter_min,
                                                                                                                  pt_diameter, pt_length, pt_volume))
    ...
    if args["visualize_model"]:
        ...
        for idx in range(N_sublist):
            ...
            for i, (sub_branch, sub_branch_start, sub_branch_radius) in enumerate(
                    zip(sub_branch_level[idx], sub_branch_start_level[idx], radius_level[idx])):
                pts = mlab.points3d(X_skeleton[sub_branch], Y_skeleton[sub_branch], Z_skeleton[sub_branch], color=color_rgb, mode='sphere',
                                    scale_factor=0.05)

                pts = mlab.points3d(X_skeleton[sub_branch_start], Y_skeleton[sub_branch_start], Z_skeleton[sub_branch_start], color=(1, 1, 1),
                                    mode='sphere', scale_factor=0.06)

                pts = mlab.text3d(X_skeleton[sub_branch_start], Y_skeleton[sub_branch_start], Z_skeleton[sub_branch_start] - 0.05,
                                  str("{:.2f}".format(sub_branch_radius)), color=(0, 1, 0), scale=(0.04, 0.04, 0.04))
...
if __name__ == '__main__':
    ...
    ap.add_argument("-o", "--output", required=False, default=None, help="Directory to save results in")
    ap.add_argument("-v", "--visualize_model", required=False, default=False, help="Display model or not, default no")
    args = vars(ap.parse_args())

    # setting path to model file
    skeleton_model_path = args["model_skeleton"]
    pcloud_model_path = args["model_pcloud"]
    ...
    label_path = Path(join(output_directory, 'label'))
    label_path.mkdir(exist_ok=True)

    print("Analyzing 3D skeleton and structure ...\n")

    # obtain image file list
    imgList = sorted(glob.glob(slice_path + ".png"))

    n_images = len(imgList)
    if n_images == 0:
        print(f"No images in {slice_path}!")
        sys.exit(1)
    ...
    (s_diameter_max, s_diameter_min, s_diameter, s_length, pt_eccentricity, avg_radius_stem, avg_density, \
     ...
     num_crown, avg_crown_length, avg_crown_angle, avg_radius_crown, avg_crown_projection, \
     avg_radius_lateral, \
     count_wholrs, whorl_dis_1, whorl_dis_2, avg_volume) = analyze_skeleton(skeleton_model_path, pcloud_model_path)
    ...
    trait_file = join(output_directory, 'trait.xlsx')
